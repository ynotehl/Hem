      ...
      build.onLoad(
        ...
        async ({ path: filename }) => {
          ...
          try {
            ...
            const contents = result.map ? `${result.code}//# sourceMappingURL=${result.map.toUrl()}` : result.code;
            return { contents };
          } catch (error) {
            const text = error instanceof Error ? error.message : String(error);
            const position = typeof error === "object" && error !== null && "position" in error ? error.position : void 0;
            return {
              errors: [
                {
                  text,
                  location: position ? { file: filename, line: position.line, column: position.column } : void 0
                }
              ]
            };
          }
...
export {
  createSvelteOptimizeEsbuildPlugins
};
